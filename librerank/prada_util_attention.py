...
def attention_summary(key_masks, key_len, query_len, outputs, scope,
    ...
    if atten_weights_collections:
        for i in range(key_len):
            for collection in atten_weights_collections:
                tf.compat.v1.add_to_collection(collection, tf.slice(weight_mean, [i], [1], name='%s_atten_weight_%d' % (scope, i)))
    ...
    tf.compat.v1.summary.image("%s_attention" % (scope), report_image[:, :80, :80, :])  # 1x10x10x1
...
def multihead_attention(queries, keys, num_units=None, num_output_units=None,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        if linear_projection:
            ...
            Q = _layers.fully_connected(queries_2d,
                                       num_units,
                                       activation_fn=None,
        ...
        if atten_mode == 'cos':
            ...
            Q_ = LayerNormalization(axis=-1, center=True, scale=True)(Q_)
            K_ = LayerNormalization(axis=-1, center=True, scale=True)(K_)
            # Multiplication
            ...
            d_layer_1_all = _layers.fully_connected(din_all, 80, activation_fn=tf.sigmoid, scope='f1_att')
            d_layer_2_all = _layers.fully_connected(d_layer_1_all, 40, activation_fn=tf.sigmoid, scope='f2_att')
            d_layer_3_all = _layers.fully_connected(d_layer_2_all, 1, activation_fn=None, scope='f3_att')
        ...
        if not is_target_attention:
            ...
            outputs = tf.compat.v1.where(tf.reshape(query_masks, [-1]), outputs,
