...
class CMR_generator(RLModel):
    ...
    def attention_based_decoder(self, decoder_inputs, initial_state, attention_states, cell, sampling_function,
        ...
        with tf.compat.v1.variable_scope(scope or "point_decoder"):
            ...
            attn_size = attention_states.get_shape()[2] # sửa ở đây  # state_size——rnn output size
                ...
                for a in range(attention_head_nums):
                    with tf.compat.v1.variable_scope("Attention_%d" % a):
                        if not self.is_controllable:
                            ...
                            # dense_layer = Dense(attention_vec_size, use_bias=True)
                            # y = dense_layer(query)
            ...
            for i, inp in enumerate(decoder_inputs):  # [(N+1)*[B,input_size]], input into decoder N by N
                ...
                if feed_context_vector:
                    ...
                    dense_layer = Dense(input_size, use_bias=True)
                    x = dense_layer(tf.concat([inp] + attns, axis=-1))
                # Run the RNN.
    ...
    def build_decoder_input(self):
        ...
        decoder_inputs = [tf.zeros([self.batch_size, self.enc_input.shape[-1]])] * ( # sửa ở đây
    ...
    def get_sampling_function(self):
        ...
        def sampling_function(attention_weights, _):
            ...
            if self.use_masking:
                attention_weights = tf.compat.v1.where(self.inference_sampled_symbol > 0, self.neg_inf,
            ...
            if self.sample_manner == "greedy":
                ...
                # 2、sample
                sampling_symbol = tf.squeeze(tf.random.categorical(tf.math.log(attention_weights), 1), axis=-1)  # [B,N] -> [B]
            ...
            if self.use_masking:
                ...
                # ***** #
                sampling_symbol_onehot = tf.compat.v1.where(self.inference_sampled_symbol > 0,
    ...
    def get_training_sampling_function(self):
        ...
        def sampling_function(attention_weights, _):
            attention_weights = attention_weights
            if self.use_masking:
                attention_weights = tf.compat.v1.where(self.training_sampled_symbol > 0, self.neg_inf, attention_weights)  # [B,N]
            attention_weights = tf.nn.softmax(attention_weights)
            ...
            if self.use_masking:
                ...
                sampling_symbol_onehot = tf.compat.v1.where(self.training_sampled_symbol > 0,
                                                  tf.zeros_like(sampling_symbol_onehot),
                                                  sampling_symbol_onehot)  # [B,N]
                # self.print_loss = tf.print("sampling_symbol", sampling_symbol,
                #                            "\nsampling_symbol_onehot", sampling_symbol_onehot,
    ...
    def _build_graph(self):
        ...
        with tf.compat.v1.variable_scope("input"):
            ...
            self.batch_size = 16 #self.dec_input.get_shape()[0].value
            ...
            self.feed_train_order = tf.compat.v1.placeholder(tf.bool)
            self.name = 'CMR_generator'
            self.train_order = tf.compat.v1.placeholder(tf.int64, [None, self.item_size])

            # self.penalty = []
            # self.loss_tf_per_sample = []
            # self.loss_tf = []
            # self.loss_numpy_per_sample = []
            # self.loss_per_sample = []
            # self.loss_torch = []
            # self.loss_numpy = []
            # self.penalty = []
            # self.mo_opt = HvMaximization(16, 2, (2,2))
        ...
        with tf.compat.v1.variable_scope("encoder_state"):
            ...
            self.decoder_cell = tf.compat.v1.nn.rnn_cell.GRUCell(self.lstm_hidden_units)
    ...
    def _build_loss(self):
        ...
        if self.loss_type == 'ce':
            ...
            reinforce_weight = tf.compat.v1.where(reinforce_weight >= 0, tf.pow(gamma, reinforce_weight),
            ...
            div_ce = tf.multiply(div_label, tf.reshape(tf.nn.softmax_cross_entropy_with_logits(logits=logits,
                                                                                               labels=tf.stop_gradient(labels)), [-1, 1]))
            ...
            # loss_per_sample = tf.stack([tf.reduce_sum(auc_ce, axis=1),tf.reduce_sum(div_ce, axis=1)])

            self.div_loss = tf.reduce_mean(tf.reduce_sum(div_ce, axis=1))
            self.auc_loss = tf.reduce_mean(tf.reduce_sum(auc_ce, axis=1))
            
            # loss_mean = tf.stack([self.auc_loss,self.div_loss])
            # self.loss_tf.append(loss_mean)
            # print(loss_mean.shape)
            # print(loss_mean)
            # self.loss_numpy.append(loss_mean)
            # self.penalty.append(tf.reduce_sum(loss_mean * self.controllable_prefer_vector)/tf.sqrt(tf.reduce_sum(loss_mean **2)) * tf.sqrt(tf.reduce_sum(self.controllable_prefer_vector**2)))
    ...
    # def _build_loss_multisample(self):
    #     self.loss_numpy = np.array(self.loss_numpy).T
    #     self.loss_numpy = self.loss_numpy[np.newaxis, :, :]
    #     n_samples, n_mo_obj, n_mo_sol = self.loss_numpy.shape
    #     dynamic_weights_per_sample = tf.ones(n_mo_sol, n_mo_obj, n_samples)
    #     for i_sample in range(0, n_samples):
    #         weights_task = self.mo_opt.compute_weights(self.loss_numpy[i_sample,:,:])
    #         dynamic_weights_per_sample[:, :, i_sample] = weights_task.permute(1,0)

    #     i_mo_sol = 0

    #     total_dynamic_loss = tf.mean(tf.reduce_sum(dynamic_weights_per_sample[i_mo_sol, :, :]
    #                                                 * self.loss_tf[i_mo_sol], axis=0))
    #     for i_mo_sol in range(1, n_mo_sol):
    #         total_dynamic_loss += tf.mean(tf.reduce_sum(dynamic_weights_per_sample[i_mo_sol, :, :] 
    #                                             * self.loss_tf[i_mo_sol], axis=0))

    #     for phat in self.penalty:
    #         self.total_dynamic_loss -= 4*phat
    ...
    #     with self.graph.as_default():
    #         auc_loss, div_loss, training_attention_distribution, training_prediction_order, predictions = \
    #             self.sess.run(
    #                 [ self.train_step_],
    #                 )
            # return  auc_loss, div_loss
