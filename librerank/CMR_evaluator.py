...
class CMR_evaluator(EGR_evaluator):
    ...
    def build_loss(self):
        with tf.compat.v1.name_scope("CMR_evaluator_Loss_Op"):
            if self.score_format == 'pv':
                ...
                if self.label_type == "total_num":  # label_ph: [B, N(0 or 1)]
                                                      ...
                                                      loss_weight = tf.compat.v1.where(loss_weight > 1, loss_weight, tf.ones_like(loss_weight))  # [B,1]
                ...
                self.loss = tf.compat.v1.losses.log_loss(self.label_ph, self.logits)
                self.gap = self.loss
    ...
    def dnn_layer(self):
        ...
        if self.feature_batch_norm:
            with tf.compat.v1.variable_scope(name_or_scope="{}_Input_BatchNorm".format(self.name)):
                # dnn_layer = tf.contrib.layers.batch_norm(dnn_layer, is_training=self.is_train, scale=True)
                dnn_layer = BatchNormalization()(dnn_layer, training=self.is_train)
    ...
    def sum_pooling_channel(self):
        with tf.compat.v1.variable_scope(name_or_scope="{}_Sum_Pooling_Channel".format(self.name)):
            self.sum_pooling_layer = tf.reduce_sum(self.all_feature_concatenation, axis=1)

    def concatenation_channel(self):
        with tf.compat.v1.variable_scope(name_or_scope="{}_Concatenation_Channel".format(self.name)) as scope:
            running_layer = _layers.fully_connected(
                self.all_feature_concatenation,
                16,
                tf.nn.relu,
                scope=scope,
                # normalizer_fn=layers.batch_norm if self.use_BN else None,
                normalizer_fn = BatchNormalization() if self.use_BN else None,
    ...
    def multi_head_self_attention_channel(self):
        with tf.compat.v1.variable_scope(name_or_scope="{}_Multi_Head_Self_Attention_Channel".format(self.name)):
    ...
    def rnn_channel(self):
        with tf.compat.v1.variable_scope(name_or_scope="{}_RNN_Channel".format(self.name)):
            # one can reverse self.all_feature_concatenation and make it a Bi-GRU
            encoder_cell = tf.compat.v1.nn.rnn_cell.GRUCell(64)
    ...
    def pair_wise_comparison_channel(self):
        with tf.compat.v1.variable_scope(name_or_scope="{}_Pair_Wise_Comparison_Channel".format(self.name)):
    ...
    def logits_layer(self):
        ...
        if self.score_format == 'pv':
            ...
            dense_layer = Dense(self.max_time_len)
            logits = dense_layer(self.final_neurons[-1])
