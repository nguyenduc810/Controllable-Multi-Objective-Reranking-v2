...
# from tensorflow.compat.v1 import static_bidirectional_rnn
from tensorflow.keras.layers import Bidirectional as static_bidirectional_rnn
...
def tau_function(x):
    return tf.compat.v1.where(x > 0, tf.exp(x), tf.zeros_like(x))
...
class MyLRSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
  ...
  def __init__(self, initial_learning_rate, decay_factor =3000):
    self.initial_learning_rate = initial_learning_rate
    self.decay_factor = decay_factor
  def __call__(self, step):
     epoch = step // self.decay_factor
...
class BaseModel(object):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        with self.graph.as_default():
            ...
            with tf.compat.v1.name_scope('inputs'):
                ...
                self.controllable_prefer_vector = tf.compat.v1.placeholder(tf.float32, [1, 2])
                self.itm_spar_ph = tf.compat.v1.placeholder(tf.int32, [None, max_time_len, itm_spar_num], name='item_spar')
                self.itm_dens_ph = tf.compat.v1.placeholder(tf.float32, [None, max_time_len, itm_dens_num], name='item_dens')
                self.usr_profile = tf.compat.v1.placeholder(tf.int32, [None, profile_num], name='usr_profile')
                ...
                self.seq_length_ph = tf.compat.v1.placeholder(tf.int32, [None, ], name='seq_length_ph')
                # self.hist_length_ph = tf.placeholder(tf.int32, [None, ], name='hist_length_ph')
                self.label_ph = tf.compat.v1.placeholder(tf.float32, [None, max_time_len], name='label_ph')
                # self.time_ph = tf.placeholder(tf.float32, [None, max_seq_len], name='time_ph')
                self.is_train = tf.compat.v1.placeholder(tf.bool, [], name='is_train')
                self.cate_id = tf.compat.v1.placeholder(tf.int32, [None, max_time_len], name='cate_id')
                
                # controllable_auc_prefer
                self.controllable_auc_prefer = tf.compat.v1.placeholder(tf.float32, [])
                # lr
                self.lr = tf.compat.v1.placeholder(tf.float32, [])
                # reg lambda
                self.reg_lambda = tf.compat.v1.placeholder(tf.float32, [])
                # keep prob
                ...
                # self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=MyLRSchedule(self.lr))
    ...
    def divide_mean_normalization(self, raw_feature):
        raw_feature_mean = tf.reduce_mean(raw_feature, axis=1, keepdims=True)  # (B, 1, emb_dims)
    ...
    def min_max_normalization(self, raw_feature, tensor_global_min_tile, tensor_global_max_tile):
        matrix_f_global = tf.compat.v1.where(tensor_global_max_tile - tensor_global_min_tile < self.epsilon,
                                   tf.fill(tf.shape(raw_feature), 0.5),
                                   tf.compat.v1.div(tf.subtract(raw_feature, tensor_global_min_tile),
                                          tf.subtract(tensor_global_max_tile,
                                                      tensor_global_min_tile) + self.epsilon))  # (B, N, emb_dims)
        return matrix_f_global

    def feature_augmentation(self):
        with tf.compat.v1.variable_scope(name_or_scope="Feature_Augmentation"):
            if False:
            ...
            self.all_feature_concatenation *= tf.tile(mask,
                                                      [1, 1, self.all_feature_concatenation.get_shape()[-1]]) # sửa ,value ở đây
    ...
    def augment_context_features(self, raw_feature):
        with tf.compat.v1.name_scope("{}_Context_Augmentation".format(self.name)):
            N = tf.shape(raw_feature)[1]
            tensor_global_max = tf.reduce_max(raw_feature, axis=1, keepdims=True)  # (B, 1, d2)
            tensor_global_min = tf.reduce_min(raw_feature, axis=1, keepdims=True)  # (B, 1, d2)
            ...
            tensor_global_mean = tf.divide(tf.reduce_sum(raw_feature, axis=1, keepdims=True),
            ...
            tensor_global_sigma_tile = tf.compat.v1.where(tf.equal(tensor_global_sigma_tile, 0),
                                                tensor_global_sigma_tile + self.epsilon,
                                                tensor_global_sigma_tile)

            raw_feature_pv_norm = tf.compat.v1.where(tf.sqrt(tensor_global_sigma_tile) < self.epsilon,
    ...
    def augment_context_features_with_mask(self, raw_feature):
        ...
        inf_mask = tf.compat.v1.where(tf.equal(1 - mask, 0), tf.fill(tf.shape(raw_feature), 0.0),
                            tf.fill(tf.shape(raw_feature), float('inf')))
        N = tf.shape(raw_feature)[1]
        seq_len_num = tf.cast(
            tf.tile(tf.reshape(self.seq_length_ph, [-1, 1, 1]), [1, 1, raw_feature.get_shape()[-1].value])
            , dtype=tf.float32)
        tensor_global_max = tf.reduce_max(raw_feature - inf_mask, axis=1, keepdims=True)  # (B, 1, d2)
        # return inf_mask
        tensor_global_min = tf.reduce_min(raw_feature + inf_mask, axis=1, keepdims=True)  # (B, 1, d2)
        tensor_global_max_tile = tf.tile(tensor_global_max, [1, N, 1])  # (B, N, d2)
        tensor_global_min_tile = tf.tile(tensor_global_min, [1, N, 1])  # (B, N, d2)

        tensor_global_mean = tf.divide(tf.reduce_sum(raw_feature * mask, axis=1, keepdims=True),
    ...
    def build_fc_net(self, inp, scope='fc'):
        with tf.compat.v1.variable_scope(scope):
            bn1 = tf.compat.v1.layers.batch_normalization(inputs=inp, name='bn1', training=self.is_train)
            fc1 = tf.compat.v1.layers.dense(bn1, 200, activation=tf.nn.relu, name='fc1')
            dp1 = tf.nn.dropout(fc1, rate=1 - (self.keep_prob), name='dp1')
            fc2 = tf.compat.v1.layers.dense(dp1, 80, activation=tf.nn.relu, name='fc2')
            dp2 = tf.nn.dropout(fc2, rate=1 - (self.keep_prob), name='dp2')
            fc3 = tf.compat.v1.layers.dense(dp2, 2, activation=None, name='fc3')
    ...
    def build_mlp_net(self, inp, layer=(500, 200, 80), scope='mlp'):
        with tf.compat.v1.variable_scope(scope):
            inp = tf.compat.v1.layers.batch_normalization(inputs=inp, name='mlp_bn', training=self.is_train)
            for i, hidden_num in enumerate(layer):
                fc = tf.compat.v1.layers.dense(inp, hidden_num, activation=tf.nn.relu, name='fc' + str(i))
                inp = tf.nn.dropout(fc, rate=1 - (self.keep_prob), name='dp' + str(i))
            final = tf.compat.v1.layers.dense(inp, 1, activation=None, name='fc_final')
    ...
    def build_hyper_mlp_net(self, inp, inp_last_dim, units, activation=tf.nn.relu):  # [B, N, ft_num]
        ...
        hyper_w = tf.reshape(tf.compat.v1.layers.dense(self.controllable_prefer_vector, w_output_dim, reuse=tf.compat.v1.AUTO_REUSE),
                             [-1, units])
        hyper_b = tf.compat.v1.layers.dense(self.controllable_prefer_vector, units, reuse=tf.compat.v1.AUTO_REUSE)
    ...
    def build_logloss(self, y_pred):
        # loss
        self.loss = tf.multiply(tf.compat.v1.losses.log_loss(self.label_ph, y_pred),
    ...
    def build_norm_logloss(self, y_pred):
        self.loss = - tf.reduce_sum(
            self.label_ph / (tf.reduce_sum(self.label_ph, axis=-1, keepdims=True) + 1e-8) * tf.math.log(y_pred))
        self.opt()

    def build_mseloss(self, y_pred):
        self.loss = tf.compat.v1.losses.mean_squared_error(self.label_ph, y_pred)
        self.opt()
    ...
    def build_attention_loss(self, y_pred):
        ...
        # self.pred_wt = y_pred
        self.loss = tf.compat.v1.losses.log_loss(self.label_wt, self.pred_wt)
    ...
    #     for v in tf.compat.v1.trainable_variables():
    #         if 'bias' not in v.name and 'emb' not in v.name:
    #             self.total_dynamic_loss += self.reg_lambda * tf.nn.l2_loss(v)
    #             # self.loss += self.reg_lambda * tf.norm(v, ord=1)

    #     self.optimizer = tf.compat.v1.train.AdamOptimizer(self.lr)

    #     if self.max_grad_norm > 0:
    #         grads_and_vars = self.optimizer.compute_gradients(self.total_dynamic_loss)
    #         for idx, (grad, var) in enumerate(grads_and_vars):
    #             if grad is not None:
    #                 grads_and_vars[idx] = (tf.clip_by_norm(grad, self.max_grad_norm), var)
    #         self.train_step_ = self.optimizer.apply_gradients(grads_and_vars)
    #     else:
    #         self.train_step_ = self.optimizer.minimize(self.total_dynamic_loss)
    ...
    def positionwise_feed_forward(self, inp, d_hid, d_inner_hid, dropout=0.9):
        with tf.compat.v1.variable_scope('pos_ff'):
            inp = tf.compat.v1.layers.batch_normalization(inputs=inp, name='bn1', training=self.is_train)
            l1 = tf.compat.v1.layers.conv1d(inp, d_inner_hid, 1, activation='relu')
            l2 = tf.compat.v1.layers.conv1d(l1, d_hid, 1)
            dp = tf.nn.dropout(l2, rate=1 - (dropout), name='dp')
            dp = dp + inp
            output = tf.compat.v1.layers.batch_normalization(inputs=dp, name='bn2', training=self.is_train)
        return output

    def bilstm(self, inp, hidden_size, scope='bilstm', reuse=False):
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            lstm_fw_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_size, forget_bias=1.0, name='cell_fw')
            lstm_bw_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_size, forget_bias=1.0, name='cell_bw')
    ...
    def load(self, path):
        # vars_to_restore_name  = ['encoder/enc_dnn_1/layer_0/weights:0', 'encoder/enc_dnn_1/layer_0/biases:0',
        #                     'encoder/enc_dnn_2/layer_0/weights:0', 'encoder/enc_dnn_2/layer_0/biases:0',
        #                     'decoder/decoder/point_decoder/Attention_0/dec_dnn/layer_0/weights:0','decoder/decoder/point_decoder/Attention_0/dec_dnn/layer_0/biases:0',
        #                      'decoder/decoder/point_decoder/Attention_0/dec_dnn/layer_1/weights:0','decoder/decoder/point_decoder/Attention_0/dec_dnn/layer_1/biases:0',
        #                       'decoder/decoder/point_decoder/Attention_1/dec_dnn/layer_0/weights:0', 'decoder/decoder/point_decoder/Attention_1/dec_dnn/layer_0/biases:0',
        #                        'decoder/decoder/point_decoder/Attention_1/dec_dnn/layer_1/weights:0','decoder/decoder/point_decoder/Attention_1/dec_dnn/layer_1/biases:0' ]
    ...
    def build_gsf_fc_function(self, inp, hidden_size, activation, scope="gsf_nn"):
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            for j in range(len(hidden_size)):
                bn = tf.compat.v1.layers.batch_normalization(inputs=inp, name='bn' + str(j), training=self.is_train)
                if j != len(hidden_size) - 1:
                    inp = tf.compat.v1.layers.dense(bn, hidden_size[j], activation=activation, name='fc' + str(j))
                else:
                    inp = tf.compat.v1.layers.dense(bn, hidden_size[j], activation=tf.nn.sigmoid, name='fc' + str(j))
    ...
    def build_miDNN_net(self, inp, layer, scope='mlp'):
        with tf.compat.v1.variable_scope(scope):
            ...
            for i, hidden_num in enumerate(layer):
                fc = tf.compat.v1.layers.dense(inp, hidden_num, activation=tf.nn.relu, name='fc' + str(i))
                inp = tf.nn.dropout(fc, rate=1 - (self.keep_prob), name='dp' + str(i))
            final = tf.compat.v1.layers.dense(inp, 1, activation=tf.nn.sigmoid, name='fc_final')
            score = tf.reshape(final, [-1, self.max_time_len])
            seq_mask = tf.sequence_mask(self.seq_length_ph, maxlen=self.max_time_len, dtype=tf.float32)
            y_pred = seq_mask * score
        return y_pred
...
class PRM(BaseModel):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        with self.graph.as_default():
            ...
            self.pos_mtx = tf.compat.v1.get_variable("pos_mtx", [max_time_len, pos_dim],
                                           initializer=tf.compat.v1.truncated_normal_initializer)
    ...
    def build_prm_fc_function(self, inp):
        ...
        if not self.is_controllable:
            fc1 = tf.compat.v1.layers.dense(bn1, self.d_model, activation=tf.nn.relu, name='fc1')
        else:
        ...
        dp1 = tf.nn.dropout(fc1, rate=1 - (self.keep_prob), name='dp1')
        if not self.is_controllable:
            fc2 = tf.compat.v1.layers.dense(dp1, 1, activation=None, name='fc2')
        else:
...
class DLCM(BaseModel):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        with self.graph.as_default():
            with tf.compat.v1.name_scope('gru'):
                seq_ht, seq_final_state = tf.compat.v1.nn.dynamic_rnn(GRUCell(hidden_size), inputs=self.item_seq,
    ...
    def build_phi_function(self, seq_ht, seq_final_state, hidden_size):
        bn1 = tf.compat.v1.layers.batch_normalization(inputs=seq_final_state, name='bn1', training=self.is_train)
        seq_final_fc = tf.compat.v1.layers.dense(bn1, hidden_size, activation=tf.nn.tanh, name='fc1')
        dp1 = tf.nn.dropout(seq_final_fc, rate=1 - (self.keep_prob), name='dp1')
        seq_final_fc = tf.expand_dims(dp1, axis=1)
        bn2 = tf.compat.v1.layers.batch_normalization(inputs=seq_ht, name='bn2', training=self.is_train)
        # fc2 = tf.layers.dense(tf.multiply(bn2, seq_final_fc), 2, activation=None, name='fc2')
        # score = tf.nn.softmax(fc2)
        # score = tf.reshape(score[:, :, 0], [-1, self.max_time_len])
        fc2 = tf.compat.v1.layers.dense(tf.multiply(bn2, seq_final_fc), 1, activation=None, name='fc2')
        score = tf.reshape(fc2, [-1, self.max_time_len])
        # sequence mask
        seq_mask = tf.sequence_mask(self.seq_length_ph, maxlen=self.max_time_len, dtype=tf.float32)
        score = score * seq_mask
        score = score - tf.reduce_min(score, 1, keepdims=True)
        return score
...
class EGR_base(BaseModel):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        with self.graph.as_default():
            ...
            with tf.compat.v1.variable_scope("network"):
                ...
                new_dense_feature, final_state = tf.compat.v1.nn.dynamic_rnn(GRUCell(hidden_size), inputs=layer1,
                                                                   sequence_length=self.seq_length_ph, dtype=tf.float32,
                                                                   scope='gru')
    ...
    def get_global_feature(self, inputph):
        ...
        matrix_f_global = tf.compat.v1.where(tf.equal(tensor_global_max_tile, tensor_global_min_tile),
                                   tf.fill(tf.shape(inputph), 0.5),
                                   tf.compat.v1.div(tf.subtract(inputph, tensor_global_min_tile),
...
class EGR_discriminator(EGR_base):
    def __init__(self, feature_size, eb_dim, hidden_size, max_time_len, itm_spar_num, itm_dens_num,
        ...
        super(EGR_discriminator, self).__init__(feature_size, eb_dim, hidden_size, max_time_len,
                                                itm_spar_num, itm_dens_num, profile_num, max_norm)
        with self.graph.as_default():
            self.d_reward = -tf.math.log(1 - self.y_pred + 1e-8)
    ...
    def build_discrim_loss(self, logits, c_entropy_d):
        ...
        self.d_loss = -tf.reduce_mean(
            y_ * tf.math.log(tf.clip_by_value(y, 1e-10, 1.0)) + (1 - y_) * tf.math.log(tf.clip_by_value(1 - y, 1e-10, 1.0)))
