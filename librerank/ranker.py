...
class BaseModel(object):
    def __init__(self, eb_dim, feature_size, itm_spar_fnum, itm_dens_fnum, user_fnum, num_item):
        ...
        with tf.compat.v1.name_scope('inputs'):
            self.user_behavior_length_ph = tf.compat.v1.placeholder(tf.int32, [None, ], name='user_behavior_length_ph')
            self.target_user_ph = tf.compat.v1.placeholder(tf.int32, [None, user_fnum], name='target_user_ph')
            self.item_spar_ph = tf.compat.v1.placeholder(tf.int32, [None, itm_spar_fnum], name='item_spar_ph')
            self.item_dens_ph = tf.compat.v1.placeholder(tf.float32, [None, itm_dens_fnum], name='item_dens_ph')
            self.label_ph = tf.compat.v1.placeholder(tf.int32, [None, ], name='label_ph')
        ...
        with tf.compat.v1.name_scope('embedding'):
            self.emb_mtx = tf.compat.v1.get_variable('emb_mtx', [feature_size, eb_dim],
    ...
    def build_mlp_net(self, inp, layer=(500, 200, 80)):
        for i, hidden_num in enumerate(layer):
            bn = tf.compat.v1.layers.batch_normalization(inputs=inp, name='bn' + str(i))
            fc = tf.compat.v1.layers.dense(bn, hidden_num, activation=tf.nn.relu, name='fc' + str(i))
            inp = tf.nn.dropout(fc, rate=1 - (self.keep_prob), name='dp' + str(i))
        final = tf.compat.v1.layers.dense(inp, 2, activation=None, name='fc_final')
    ...
    def build_logloss(self):
        # loss
        self.log_loss = tf.compat.v1.losses.log_loss(self.label_ph, self.y_pred)
    ...
    def build_mseloss(self):
        self.loss = tf.compat.v1.losses.mean_squared_error(self.label_ph, self.y_pred)
        # regularization term
        for v in tf.compat.v1.trainable_variables():
            if 'bias' not in v.name and 'emb' not in v.name:
                self.loss += self.reg_lambda * tf.nn.l2_loss(v)
